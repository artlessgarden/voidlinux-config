...
has "$installer" '"$source_bin" -t'
has "$installer" '/var/service/mihomo'
has "$installer" 'install -m 600'

validation_line=$(grep -n '"$source_bin" -t' "$installer" | head -n 1 | cut -d: -f1)
...
# A rejected configuration must leave the installed binary and service alone.
: >"$tmp/log"
if VALIDATION_EXIT=1 MIHOMO_INSTALL_LOG=$tmp/log MIHOMO_BIN=$tmp/mihomo HOME=$tmp/home \
	PATH="$tmp/bin:$PATH" sh "$installer" >/dev/null 2>&1; then
	fail 'invalid configuration was accepted'
fi
if grep -Eq '^(mv |ln |sv .*restart)' "$tmp/log"; then
	fail 'invalid configuration replaced the binary or changed the service'
fi
