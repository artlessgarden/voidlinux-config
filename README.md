...
默认浏览器为 qutebrowser（`Mod+c` → `qute-new`：已有实例走 IPC 开新窗，和窗内 `Ctrl+N` 同路径）。核显用 AMD `amdgpu`（nouveau 已屏蔽）。  
...
油猴：`~/.config/qutebrowser/greasemonkey/`（bilibili-ip）。B 站净化：`qute-gm-bili` 从上游抽出 html[attr]+CSS → `~/.local/share/qutebrowser/greasemonkey/bili-clean.user.js`；左下角「净化」或 `,b`。  
