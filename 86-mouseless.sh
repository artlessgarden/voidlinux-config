...
mkdir -p "$HOME/.local/bin" "$HOME/.config/mouseless"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
...
echo "mouseless -> $HOME/.local/bin/mouseless ($ver)"
echo "组 input+uinput 要重新登录才不用 sudo；当前可用: sudo -n mouseless --config ~/.config/mouseless/config.yaml"
