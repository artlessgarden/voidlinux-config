...
pub const Chip = struct {
    ...
    pub fn render(self: *Chip, gpa: std.mem.Allocator) void {
        ...
        self.paint(w, h, self.slot_num, title, self.pending_kind) catch return;
    ...
    fn paint(
        ...
        const stride = bw * 4;
        const bytes: usize = @intCast(stride * bh);

        if (self.map == null or self.map_len < bytes or self.width != bw or self.height != bh) {
            self.dropBuffer();
            ...
            const buffer = try pool.createBuffer(0, bw, bh, stride, .argb8888);
            self.fd = fd;
            self.map = mapped;
            self.pool = pool;
            self.buffer = buffer;
