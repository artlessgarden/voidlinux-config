...
- 2FA 条目：识别 `totp:` 密钥或 `otpauth://totp/…`，点击时本地生成并复制验证码
- 电脑和手机使用同一种河流界面
...
电脑按 `/` 后，右下角出现只负责实时过滤的搜索框；按 `Esc` 收起。按 `o` 新增。手机右下角始终显示圆形搜索按钮，点击搜索，长按新增。点击已有正文后会在原处出现多行编辑框；按 `Esc`、点击空白处或直接点击另一条都会提交，最后一种会立刻进入下一条。空白的新条目会被丢弃。浏览器崩溃时，尚未提交的草稿可能丢失。
...
默认河流就是时间入口。条目按创建日期留在原处；`@10-23` 会让它同时出现在今天下方的未来预览。若今年的 10 月 23 日已经过去，就解释为明年的 10 月 23 日。年份由查询计算，不写回正文。
...
按 `o` 新增条目，输入账户备注，再单独写一行 `totp: 你的Base32密钥`。
也可以直接粘贴验证器导出的 `otpauth://totp/…` 配置链接（独占一行）。
每条记录保存一个密钥，仍可按备注搜索、编辑，并通过原有加密流程同步。

原文照常显示，包括密钥或配置链接，下方多一个“复制验证码”按钮。
点击按钮才计算并复制当前验证码；点击正文仍像普通条目一样编辑。
删除配置行后恢复成普通文本条目。配置错误会显示提示，不生成猜测的验证码。

`totp:` 默认使用 SHA-1、6 位、30 秒。配置链接还支持 SHA256/SHA512、8 位和自定义正整数秒周期。
当前只支持时间型 TOTP，不支持 HOTP、二维码扫描或特殊厂商编码。
生成使用设备系统时间，请保持系统时间准确。复制后及时粘贴；过期时再次点击即可。

验证码在浏览器通过 Web Crypto 生成，不调用第三方服务；密钥作为条目正文加密保存，
不为验证码增加服务端明文字段。没有自动换码、倒计时或后台计算；点击复制不会触发保存或修改条目日期。
这是为其他账户生成验证码的功能，不是为 Text Vault 本身增加登录二次验证。
